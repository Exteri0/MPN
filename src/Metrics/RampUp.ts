...
export default class RampUpTime {
    ...
    static calculateRampUpTimeNPM(data: any): number {
        const { readme, Versions, Maintainers, Dependencies, repository, githubrepository } = data

        const normalizedVersions = Math.log(Versions+1) / Math.log(50+1); 

        const normalizedMaintainers = Math.log(Maintainers+1) / Math.log(10+1);

        const normalizedDependencies = 1 - (Math.log(Dependencies+1) / Math.log(20+1));

        const weightReadme = 0.25;
        const weightVersions = 0.20;
        const weightMaintainers = 0.15;
        const weightDependencies = 0.25;
        const weightGithubRepository = 0.15;

        const rampUpScore = (readme * weightReadme) +
...
(async () => {
    ...
    if (APIObj instanceof GitHubApiCalls) {
        ...
        score = await RampUpTime.calculateRampUpTimeNPM({
            ...
            githubrepository: response.repository.url.includes("github") ? 1:0,
        });
