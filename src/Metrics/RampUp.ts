...
export class RampUpTime extends Metrics{
    ...
    public async computeRampUpTime(): Promise<number> {
        ...
        if (this.apiCall instanceof GitHubApiCalls) { // We use nullish coalescing to check if certain variables might return undefined or null
            ...
            const normalizedVersions = await this.normalizeLog((response.versions?.length ?? 0), 50);
            ...
            rampUpScore = (readmeScore * weightReadme) + (normalizedVersions * weightVersions) + (normalizedMaintainers * weightMaintainers) +
    ...
    private async normalizeLog(value: number, maxValue: number): Promise<number> {
